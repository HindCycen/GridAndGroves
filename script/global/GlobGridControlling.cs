...
public partial class Glob {
    ...
    public static Vector2 FindNearestGridPoint(Vector2 targetPoint) {
        ...
        var minDistanceSquared = GridPoints[0, 0].DistanceSquaredTo(targetPoint);

        for (var col = 0; col < 7; col++) {
            for (var row = 0; row < 5; row++) {
                var currentPoint = GridPoints[col, row];
                var distanceSquared = currentPoint.DistanceSquaredTo(targetPoint);
