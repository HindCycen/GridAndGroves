...
2. **脚本创建** — 不要直接生成需要在编辑器中挂载给节点的 `.gd` 文件。描述脚本的名称、路径，由人类开发者使用编辑器创建空文件，之后你再来填充内容。直接生成的文件会存在 `.uid` 问题，无法与编辑器良好协作。如果你的脚本不需要挂载给节点，或者其本身是资源文件的性质，那么可以直接生成 `.gd` 文件（如工具类、辅助函数等）。
